...
large_image = cv2.imread('Data/test1.png')
...
loc = (np.array([], dtype=np.int64), np.array([], dtype=np.int64))
i = 1
while (loc[0].size == 0 and loc[1].size == 0) and i < 10:
    result = cv2.matchTemplate(large_gray, template_gray, cv2.TM_CCOEFF_NORMED)
    loc = np.where(result >= threshold)
    height, width = large_gray.shape[:2]
    print(loc)
    height += height // 10
    width += width // 10
    large_gray = cv2.resize(large_gray, (height, width))
    i += 1
...
for i in picks:
    ...
    cv2.imshow("Blue Pick: " + champ, crop)
    cv2.waitKey(0)
    print(champ)
